...
function atras() {
    $resp = "";
    $contador = 0;
    $sql = pg_query("select max(" . $_POST['id_tabla'] . ") + 1 from  " . $_POST['tabla'] . " where id_director='$_SESSION[id]'");
...
function adelante() {
    ...
    $sql = pg_query("select min(" . $_POST['id_tabla'] . ") from  " . $_POST['tabla'] . " where id_director='$_SESSION[id]'");
    while ($row = pg_fetch_row($sql)) {
        $contador = $row[0];
    }
