<?php
    ...
    class PDF extends FPDF{   
        ...
        function Header(){                                     
            ...
            $this->Cell(190, 5, utf8_decode("COBROS REALIZADOS EXTERNOS POR CLIENTE"),0,1, 'C',0);                                                                                                                            
            $this->SetFont('Amble-Regular','',10);        
            $this->Ln(3);
    ...
    while($row=pg_fetch_row($consulta)){
         ...
         while($row1=pg_fetch_row($sql1)){                 
            ...
            if(pg_num_rows($sql2)>0){
                if($repetido==0){
                    ...
                    $pdf->Cell(30, 6, utf8_decode('Valor Pago'),1,0, 'C',0);                                     
                ...
                while($row2=pg_fetch_row($sql2)){
                    $pdf->Cell(25, 6, utf8_decode($row2[3]),0,0, 'C',0);
                    $pdf->Cell(30, 6, utf8_decode($row2[9]),0,0, 'C',0);
                    $pdf->Cell(40, 6, $row2[8],0,0, 'C',0);
                    $pdf->Cell(20, 6, utf8_decode($row2[12] + $row2[13]),0,0, 'C',0);
                    $pdf->Cell(30, 6, utf8_decode($row2[12]),0,0, 'C',0);                    
                    $pdf->Cell(30, 6, utf8_decode($row2[13]),0,0, 'C',0); 
                    $pdf->Cell(30, 6, utf8_decode($row2[10]),0,1, 'C',0);
                    $sub=$sub+$row2[12];                                        
                    $saldo=$saldo+$row2[13];                                        
                }                
            }
        ...
        if($contador>0){
            ...
            $pdf->Cell(116, 6, utf8_decode("Totales"),0,0, 'R',0);
